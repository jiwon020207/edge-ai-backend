...
router = APIRouter(prefix="/api/v1/events/url", tags=["events"])

@router.post("/", response_model=schemas.UrlEventOut, status_code=status.HTTP_201_CREATED)
def add_url_event(evt_in: schemas.UrlEventCreate,
                  current_user=Depends(get_current_user),
                  db: Session = Depends(get_db)):
    return crud.create_url_event(db, current_user.id, evt_in)

@router.get("/", response_model=list[schemas.UrlEventOut])
